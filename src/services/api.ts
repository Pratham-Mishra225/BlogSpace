import type { AuthDTO, CreatePostDTO, Post, Profile, User } from "@/types";

const DELAY = 700;
const ERROR_RATE = 0; // bumped to 0 to keep mock UX smooth; flip to 0.05 to test errors
...
\`\`\`ts
function publish(post: { title: string; body: string }) {
...
// ---- seeded posts (mutable in-memory DB) ----
const posts: Post[] = [
  {
    id: "p1",
    title: "The Quiet Discipline of Showing Up",
    content: sampleContent(
      "We tend to romanticize inspiration. The truth is duller and more demanding: the work shows up because *you* did.",
    ),
    coverImage:
  ...
  },
  {
    ...
    content: sampleContent(
      "Software ages like architecture: gracefully if the bones are good, miserably if they are not.",
    ),
    coverImage:
  ...
  },
  {
    ...
    content: sampleContent(
      "The light here does most of the storytelling. You only have to stand still long enough to listen.",
    ),
    coverImage:
  ...
  },
  {
    ...
    content: sampleContent(
      "There is no shortage of new books. There is, however, a shortage of attention — and that is the variable worth optimizing.",
    ),
    coverImage:
  ...
  },
  {
    ...
    content: sampleContent(
      "Mornings, kept slow on purpose, become the editor of the rest of the day.",
    ),
    coverImage:
  ...
  },
  {
    ...
    content: sampleContent(
      "If you write for the feed, the feed owns your voice. Write for one person, slowly, and the rest will find you.",
    ),
    coverImage:
  ...
  },
  {
    ...
    content: sampleContent(
      "A camera, used patiently, is an excuse to stand somewhere a little longer than is comfortable.",
    ),
    coverImage:
  ...
  },
  {
    ...
    content: sampleContent(
      "A pencil in the margin is the cheapest, oldest, and most underrated reading tool ever invented.",
    ),
    coverImage:
...
export const getPosts = (): Promise<Post[]> =>
  delay([...posts].sort((a, b) => +new Date(b.createdAt) - +new Date(a.createdAt)));
...
export const toggleFollow = (userId: string): Promise<{ following: boolean }> => {
...
export const login = (data: AuthDTO): Promise<User> => {
  const user = users[0];
  return delay({ ...user, name: user.name }).then((u) => {
    void data;
    return u;
  });
};
